...
public class Skidmarks : MonoBehaviour {
	...
	protected void LateUpdate() {
		...
		if (!haveSetBounds) {
			...
			marksMesh.bounds = new Bounds(new Vector3(0, 0, 0), new Vector3(100000, 100000, 100000));
			haveSetBounds = true;
		}
	...
	public int AddSkidMark(Vector3 pos, Vector3 normal, Color32 colour, int lastIndex) {
		...
		if (lastSection != null) {
			...
			curSection.Posl = curSection.Pos + xDirection * MarkWidth * 0.5f;
			curSection.Posr = curSection.Pos - xDirection * MarkWidth * 0.5f;
			curSection.Tangent = new Vector4(xDirection.x, xDirection.y, xDirection.z, 1);

			if (lastSection.LastIndex == -1) {
				lastSection.Tangent = curSection.Tangent;
				lastSection.Posl = curSection.Pos + xDirection * MarkWidth * 0.5f;
				lastSection.Posr = curSection.Pos - xDirection * MarkWidth * 0.5f;
			}
		}
